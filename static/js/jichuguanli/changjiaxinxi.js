app.controller('ChangjiaxinxiController', function($scope, $http, $timeout,$rootScope) {
	$timeout(function(){
		//遍历按钮选出该页按钮
		var narr2 = [];  //遍历按钮
		for (var i = 0; i < $scope.btns.length; i++) {
			if($scope.btns[i].HREF=='base.supplier.supplierList'){
	...
	$scope.ref = function(){		
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/base/supplier/getList',
			data:{
				'start':0,
				'limit':8,
				's_like_s_code':'',
	...
	$scope.submitf = function(){
		...
		$http({
			...
			data:{
				'start':0,
				'limit':8,
				's_like_s_code':$scope.changjia_code,
				's_like_s_name':$scope.changjia_name,
				'equip_categ':$scope.changjia_lei
			},
	...
	$scope.edit = function(options){
		...
		if($scope.idList3.length==1){
			$scope.m1=$scope.lists.root[$scope.idList3[0]].S_CODE;
			$scope.m2=$scope.lists.root[$scope.idList3[0]].S_NAME;
		...
		if($scope.idList.length>=1){
			layer.msg('你确定删除么？', {
			  ...
			  ,yes: function(index){
				...
				method: 'POST',
				//  withCredentials: true,
				url: HTTP + 'jf/base/supplier/delete',
				data:{
					ids:$scope.idList
				},
	...
	$scope.save=function(){
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/base/supplier/save',
			data:{
				'form_ID':$scope.formId,
				'form_S_CODE':$scope.m1,
				'form_S_NAME':$scope.m2,
