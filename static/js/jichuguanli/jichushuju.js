app.controller('JichushujuController', function($scope, $http, $timeout,$rootScope) {
	$timeout(function(){
		//遍历按钮选出该页按钮
		var narr2 = [];  //遍历按钮
		for (var i = 0; i < $scope.btns.length; i++) {
			if($scope.btns[i].HREF=='base.codeData.codeList'){
	...
	$scope.ref = function(){		
		...
		$http({
			...
			url: HTTP + 'jf/base/codeData/getList',
			data:{
				'start':0,
				'limit':8,
				's_like_CODE_TYPE':'',
				's_like_CODE_TYPE_NAME':'',
				's_like_VALUE':'',
				's_like_NAME':''
			},
	...
	$scope.submitf = function(){
		...
		$http({
			...
			data:{
				'start':0,
				'limit':8,
				's_like_CODE_TYPE':$scope.code_lei,
				's_like_CODE_TYPE_NAME':$scope.code_leiname,
				's_like_VALUE':$scope.code_value,
				's_like_NAME':$scope.code_name
			},
	...
	$scope.view = function(options){
		...
		if($scope.idList3.length==1){
			$scope.m1=$scope.lists.root[$scope.idList3[0]].CODE_TYPE;
			$scope.m2=$scope.lists.root[$scope.idList3[0]].CODE_TYPE_NAME;
			$scope.m3=$scope.lists.root[$scope.idList3[0]].VALUE;
			$scope.m4=$scope.lists.root[$scope.idList3[0]].NAME;
		...
		if($scope.idList.length>=1){
			layer.msg('你确定删除么？', {
			  ...
			  ,yes: function(index){
				...
				method: 'POST',
				//  withCredentials: true,
				url: HTTP + 'jf/base/codeData/delete',
				data:{
					ids:$scope.idList
				},
	...
	$scope.save=function(){
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/base/codeData/save',
			data:{
				'form_ID':$scope.formId,
				'form_CODE_TYPE':$scope.m1,
				'form_CODE_TYPE_NAME':$scope.m2,
				'form_VALUE':$scope.m3,
				'form_NAME':$scope.m4,			
			},			
			}).then(function successCallback(response){
