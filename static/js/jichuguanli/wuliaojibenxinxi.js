app.controller('WuliaojibenxinxiController', function($scope, $http, $timeout,$rootScope) {	
	$timeout(function(){
		//遍历按钮选出该页按钮
		var narr2 = [];  //遍历按钮
		for (var i = 0; i < $scope.btns.length; i++) {
			if($scope.btns[i].HREF=='base.mtrDet.mtrDetList'){
	...
	$scope.ref = function(){		
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/base/mtrDet/getList',
			data:{
				'start':0,
				'limit':8,
				's_like_bar_code':'',
	...
	$scope.submitf = function(){
		...
		$http({
			...
			data:{
				'start':0,
				'limit':8,
				's_like_bar_code':$scope.tiaoma_name,
	...
	$scope.view = function(options){
		...
		if($scope.idList3.length==1){
			$scope.m1=$scope.lists.root[$scope.idList3[0]].BAR_CODE;
			$scope.m2=$scope.lists.root[$scope.idList3[0]].EQUIP_CATEG;
		...
		if($scope.idList.length>=1){
			layer.msg('你确定删除么？', {
			  ...
			  ,yes: function(index){
				...
				method: 'POST',
				//  withCredentials: true,
				url: HTTP + 'jf/base/mtrDet/delete',
				data:{
					ids:$scope.idList
				},
	...
	$scope.save=function(){
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/base/mtrDet/save',
			data:{
				'form_ID':$scope.formId,
				'form_BAR_CODE':$scope.m1,
				'form_EQUIP_CATEG':$scope.m2	
			},			
			}).then(function successCallback(response){
				console.log(response)
				if(response.data.success){
