app.controller('HuoweixiangweihuController', function($scope, $http, $timeout,$rootScope) {	
	$timeout(function(){
		//遍历按钮选出该页按钮
		var narr2 = [];  //遍历按钮
		for (var i = 0; i < $scope.btns.length; i++) {
			if($scope.btns[i].HREF=='wh.locBox.locBoxList'){
	...
	$scope.ref = function(){		
		...
		$http({
			...
			url: HTTP + 'jf/wh/loc/getLocBoxList',
			data:{
				'start':0,
				'limit':8,
				's_like_loc_code':'',
	...
	$scope.submitf = function(){
		...
		$http({
			...
			data:{
				'start':0,
				'limit':8,
				's_like_loc_code':$scope.huowei_no,
				's_like_bar_code':$scope.xiang_code
			},
	...
	$scope.edit = function(options){
		...
		if($scope.idList3.length==1){
			...
			$scope.m3=$scope.lists.root[$scope.idList3[0]].WH_HOME;
			$scope.m4=$scope.lists.root[$scope.idList3[0]].WH_AREA;
			$scope.m5=$scope.lists.root[$scope.idList3[0]].BAR_CODE;
	...
	//保存
	$scope.save=function(){
		if($scope.m1==''||$scope.m2==''||$scope.m3==''||$scope.m4==''||$scope.m5==''){
		...
		$http({
			method: 'POST',
			//  withCredentials: true,
			url: HTTP + 'jf/wh/loc/updateLocBox',
			data:{
